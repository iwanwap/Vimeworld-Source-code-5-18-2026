    ...
    public void calculatePitch() {
        if (this.channel != null && this.channel.attachedSource == this && this.channelJavaSound != null) {
            if (SoundSystemConfig.getDopplerFactor() == 0.0f) {
                ...
                float SS2 = 343.3f;
                Vector3D SV2 = this.velocity;
                Vector3D LV2 = this.listener.velocity;
                float DV2 = SoundSystemConfig.getDopplerVelocity();
                float DF2 = SoundSystemConfig.getDopplerFactor();
                Vector3D SL2 = this.listener.position.subtract(this.position);
                float vls = SL2.dot(LV2) / SL2.length();
                float vss = SL2.dot(SV2) / SL2.length();
                vss = this.min(vss, SS2 / DF2);
                float newPitch = this.pitch * (SS2 * DV2 - DF2 * (vls = this.min(vls, SS2 / DF2))) / (SS2 * DV2 - DF2 * vss);
                if (newPitch < 0.5f) {
                    newPitch = 0.5f;
