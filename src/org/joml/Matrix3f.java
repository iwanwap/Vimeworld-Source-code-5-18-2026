...
Matrix3fc {
    ...
    public Matrix3f set(Matrix3fc m2) {
        if (m2 == this) {
            return this;
        }
        return this._m00(m2.m00())._m01(m2.m01())._m02(m2.m02())._m10(m2.m10())._m11(m2.m11())._m12(m2.m12())._m20(m2.m20())._m21(m2.m21())._m22(m2.m22());
    }

    public Matrix3f setTransposed(Matrix3fc m2) {
        float nm10 = m2.m01();
        float nm12 = m2.m21();
        float nm20 = m2.m02();
        float nm21 = m2.m12();
    ...
    public Matrix3f set(Matrix4x3fc m2) {
        this.m00 = m2.m00();
        this.m01 = m2.m01();
        this.m02 = m2.m02();
        this.m10 = m2.m10();
    ...
    public Matrix3f set(AxisAngle4f axisAngle) {
        ...
        float invLength = Math.invsqrt(x2 * x2 + y2 * y2 + z2 * z2);
        x2 *= invLength;
        y2 *= invLength;
        z2 *= invLength;
    ...
    public Matrix3f set(Quaternionfc q2) {
        return this.rotation(q2);
    }

    public Matrix3f set(Quaterniondc q2) {
        double w2 = q2.w() * q2.w();
        double x2 = q2.x() * q2.x();
        double y2 = q2.y() * q2.y();
        double z2 = q2.z() * q2.z();
    ...
    public Matrix3f set(float[] m2) {
        return this.set(m2, 0);
    }

    public Matrix3f set(float[] m2, int off) {
    ...
    public Matrix3f invert(Matrix3f dest) {
        float a2 = Math.fma(this.m00, this.m11, -this.m01 * this.m10);
        float b2 = Math.fma(this.m02, this.m10, -this.m00 * this.m12);
        float c2 = Math.fma(this.m01, this.m12, -this.m02 * this.m11);
        float d2 = Math.fma(a2, this.m22, Math.fma(b2, this.m21, c2 * this.m20));
        float s2 = 1.0f / d2;
        float nm00 = Math.fma(this.m11, this.m22, -this.m21 * this.m12) * s2;
        float nm01 = Math.fma(this.m21, this.m02, -this.m01 * this.m22) * s2;
        float nm02 = c2 * s2;
        float nm10 = Math.fma(this.m20, this.m12, -this.m10 * this.m22) * s2;
        float nm11 = Math.fma(this.m00, this.m22, -this.m20 * this.m02) * s2;
        float nm12 = b2 * s2;
        float nm20 = Math.fma(this.m10, this.m21, -this.m20 * this.m11) * s2;
        float nm21 = Math.fma(this.m20, this.m01, -this.m00 * this.m21) * s2;
        float nm22 = a2 * s2;
        dest.m00 = nm00;
        dest.m01 = nm01;
        dest.m02 = nm02;
    ...
    public Matrix3f scale(float x2, float y2, float z2, Matrix3f dest) {
    ...
    public Matrix3f scale(float x2, float y2, float z2) {
    ...
    public Matrix3f scaleLocal(float x2, float y2, float z2, Matrix3f dest) {
    ...
    public Matrix3f scaleLocal(float x2, float y2, float z2) {
    ...
    public Matrix3f scaling(float x2, float y2, float z2) {
        MemUtil.INSTANCE.zero(this);
        this.m00 = x2;
        this.m11 = y2;
        this.m22 = z2;
    ...
    public Matrix3f rotation(float angle, float x2, float y2, float z2) {
    ...
    public Vector3f transformTranspose(Vector3f v2) {
        return v2.mulTranspose(this);
    }

    public Vector3f transformTranspose(Vector3fc v2, Vector3f dest) {
        return v2.mulTranspose(this, dest);
    }

    public Vector3f transformTranspose(float x2, float y2, float z2, Vector3f dest) {
    ...
    public Matrix3f rotate(float ang, float x2, float y2, float z2) {
        return this.rotate(ang, x2, y2, z2, this);
    }

    public Matrix3f rotate(float ang, float x2, float y2, float z2, Matrix3f dest) {
    ...
    public Matrix3f rotateLocal(float ang, float x2, float y2, float z2, Matrix3f dest) {
        float s2 = Math.sin(ang);
        float c2 = Math.cosFromSin(s2, ang);
        float C2 = 1.0f - c2;
        float xx2 = x2 * x2;
    ...
    public Matrix3f rotateLocal(float ang, float x2, float y2, float z2) {
    ...
    public Matrix3f lookAlong(Vector3fc dir, Vector3fc up2) {
        return this.lookAlong(dir.x(), dir.y(), dir.z(), up2.x(), up2.y(), up2.z(), this);
    }

    public Matrix3f lookAlong(Vector3fc dir, Vector3fc up2, Matrix3f dest) {
    ...
    public Matrix3f setLookAlong(Vector3fc dir, Vector3fc up2) {
    ...
    public Matrix3f setRow(int row, float x2, float y2, float z2) throws IndexOutOfBoundsException {
        switch (row) {
            case 0: {
                this.m00 = x2;
                this.m10 = y2;
    ...
    public Matrix3f setColumn(int column, float x2, float y2, float z2) throws IndexOutOfBoundsException {
        switch (column) {
            case 0: {
                this.m00 = x2;
                this.m01 = y2;
    ...
    public boolean equals(Matrix3fc m2, float delta) {
        if (this == m2) {
            return true;
        }
        if (m2 == null) {
    ...
    public Matrix3f setSkewSymmetric(float a2, float b2, float c2) {
        this.m22 = 0.0f;
        this.m11 = 0.0f;
        this.m00 = 0.0f;
        this.m01 = -a2;
    ...
    public Matrix3f lerp(Matrix3fc other, float t2) {
        return this.lerp(other, t2, this);
    }

    public Matrix3f lerp(Matrix3fc other, float t2, Matrix3f dest) {
    ...
    public Matrix3f rotateTowards(Vector3fc direction, Vector3fc up2, Matrix3f dest) {
        return this.rotateTowards(direction.x(), direction.y(), direction.z(), up2.x(), up2.y(), up2.z(), dest);
    }

    public Matrix3f rotateTowards(Vector3fc direction, Vector3fc up2) {
    ...
    public Matrix3f rotationTowards(Vector3fc dir, Vector3fc up2) {
    ...
    public Matrix3f obliqueZ(float a2, float b2) {
    ...
    public Matrix3f obliqueZ(float a2, float b2, Matrix3f dest) {
        dest.m00 = this.m00;
        dest.m01 = this.m01;
        dest.m02 = this.m02;
        dest.m10 = this.m10;
    ...
    public Matrix3f reflect(float nx2, float ny2, float nz2, Matrix3f dest) {
        float da2 = nx2 + nx2;
        float db2 = ny2 + ny2;
        float dc2 = nz2 + nz2;
        float rm00 = 1.0f - da2 * nx2;
        float rm01 = -da2 * ny2;
        float rm02 = -da2 * nz2;
        float rm10 = -db2 * nx2;
        float rm11 = 1.0f - db2 * ny2;
        float rm12 = -db2 * nz2;
        float rm20 = -dc2 * nx2;
        float rm21 = -dc2 * ny2;
        float rm22 = 1.0f - dc2 * nz2;
        float nm00 = this.m00 * rm00 + this.m10 * rm01 + this.m20 * rm02;
        float nm01 = this.m01 * rm00 + this.m11 * rm01 + this.m21 * rm02;
        float nm02 = this.m02 * rm00 + this.m12 * rm01 + this.m22 * rm02;
        float nm10 = this.m00 * rm10 + this.m10 * rm11 + this.m20 * rm12;
    ...
    public Matrix3f reflect(float nx2, float ny2, float nz2) {
    ...
    public Matrix3f reflection(float nx2, float ny2, float nz2) {
        float da2 = nx2 + nx2;
        float db2 = ny2 + ny2;
        float dc2 = nz2 + nz2;
        this._m00(1.0f - da2 * nx2);
        this._m01(-da2 * ny2);
        this._m02(-da2 * nz2);
        this._m10(-db2 * nx2);
        this._m11(1.0f - db2 * ny2);
        this._m12(-db2 * nz2);
        this._m20(-dc2 * nx2);
        this._m21(-dc2 * ny2);
        this._m22(1.0f - dc2 * nz2);
        return this;
    }
    ...
    public float quadraticFormProduct(float x2, float y2, float z2) {
        float Axx = this.m00 * x2 + this.m10 * y2 + this.m20 * z2;
        float Axy = this.m01 * x2 + this.m11 * y2 + this.m21 * z2;
        float Axz = this.m02 * x2 + this.m12 * y2 + this.m22 * z2;
        return x2 * Axx + y2 * Axy + z2 * Axz;
    }

    public float quadraticFormProduct(Vector3fc v2) {
