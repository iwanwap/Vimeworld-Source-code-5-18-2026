...
Serializable {
    ...
    public boolean contains(int X2, int Y2) {
        ...
        h2 += y2;
        return !((w2 += x2) >= x2 && w2 <= X2 || h2 >= y2 && h2 <= Y2);
    }
    ...
    public boolean contains(int X2, int Y2, int W2, int H2) {
        int w2 = this.width;
        int h2 = this.height;
        if ((w2 | h2 | W2 | H2) < 0) {
            return false;
        ...
        w2 += x2;
        if ((W2 += X2) <= X2 ? w2 >= x2 || W2 > w2 : w2 >= x2 && W2 > w2) {
            return false;
        }
        h2 += y2;
        return !((H2 += Y2) <= Y2 ? h2 >= y2 || H2 > h2 : h2 >= y2 && H2 > h2);
    }
    ...
    public boolean intersects(ReadableRectangle r2) {
        ...
        tw2 += tx2;
        th2 += ty2;
        return !((rw2 += rx2) >= rx2 && rw2 <= tx2 || rh2 >= ry2 && rh2 <= ty2 || tw2 >= tx2 && tw2 <= rx2 || th2 >= ty2 && th2 <= ry2);
    }
