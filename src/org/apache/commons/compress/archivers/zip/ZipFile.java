...
implements Closeable {
    ...
    public InputStream getInputStream(ZipArchiveEntry ze2) throws IOException, ZipException {
        ...
        BoundedInputStream bis = new BoundedInputStream(start, ze2.getCompressedSize());
        switch (ZipMethod.getMethodByCode(ze2.getMethod())) {
            ...
            case IMPLODING: {
                return new ExplodingInputStream(ze2.getGeneralPurposeBit().getSlidingDictionarySize(), ze2.getGeneralPurposeBit().getNumberOfShannonFanoTrees(), new BufferedInputStream(bis));
            }
            case DEFLATED: {
                bis.addDummy();
