...
implements ClientConnectionManager {
    ...
    public void releaseConnection(ManagedClientConnection conn, long validDuration, TimeUnit timeUnit) {
        ...
        BasicPooledConnAdapter hca2 = (BasicPooledConnAdapter)conn;
        if (hca2.getPoolEntry() != null) {
            Asserts.check(hca2.getManager() == this, "Connection not obtained from this manager");
        }
        BasicPooledConnAdapter basicPooledConnAdapter = hca2;
        synchronized (basicPooledConnAdapter) {
            BasicPoolEntry entry = (BasicPoolEntry)hca2.getPoolEntry();
            if (entry == null) {
                return;
            }
            try {
                if (hca2.isOpen() && !hca2.isMarkedReusable()) {
                    hca2.shutdown();
                }
            }
                ...
                boolean reusable = hca2.isMarkedReusable();
                if (this.log.isDebugEnabled()) {
                    if (reusable) {
