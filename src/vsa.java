/*
 * Decompiled with CFR 0.152.
 */
public final class vsa {
    public static final String ja;
    public static final String Ha;
    public static final String Ga;
    public static final int fa;
    public static final long Ka;
    public static final String Aa;
    public static final int S;
    public static final String Z;
    public static final int Y;
    public static final String r;
    public static final String V;
    public static final float z;
    public static final String X;
    public static final String y;
    public static final String U;
    public static final String P;
    public static final String u;
    public static final String O;
    public static final String t;
    public static final String R;
    public static final String Q;
    public static final String N;
    public static final String T;
    public static final String p;
    public static final String n;
    public static final String s;
    public static final String w;
    public static final String W;
    public static final String q;
    public static final String x;
    public static final String v;
    public static final String o;
    public static final String h;
    public static final double K;
    public static final String H;
    public static final String c;
    public static final String B;
    public static final String d;
    public static final String a;
    public static final String b;
    public static final String l;
    public static final String e;
    public static final String G;
    public static final String D;
    public static final String f;
    public static final String F;
    public static final String g;
    public static final String L;
    public static final String E;
    public static final String m;
    public static final String C;
    public static final int i;
    public static final String M;
    public static final String k;
    public static final String j;
    public static final float J;
    public static final String A;
    public static final String I;

    public vsa() {
        vsa a2;
    }

    static {
        t = "getBrightness";
        Y = 3170 /* ZKM Deobf Int */;
        K = 0.32532500000000003D /* ZKM Deobf */;
        J = 0.015625F /* ZKM Deobf */;
        x = "timerDuration_";
        T = "eBeam_";
        F = "rabbit_foot";
        Ka = 0xB91BA0F2C02438FFL ^ 0x86E45F0D3FDBC700L;
        fa = 34192 /* ZKM Deobf Int */;
        g = "polishedTuffStairs";
        R = "HeightMap";
        w = "EASE_IN_BACK";
        p = "shadowcolor";
        b = "GL11.GL_3D_COLOR";
        A = "stat.noteblockTuned";
        q = "textures/pet/bomb.png";
        o = "\\[[-+\\d|,\\s]+\\]";
        h = "BLACK";
        i = 1237 /* ZKM Deobf Int */;
        y = "goalTick";
        O = "commands.clone.success";
        D = "BEACON";
        u = "GL13.GL_MULTISAMPLE";
        Ha = "bottom";
        d = "maxFps:";
        c = "of.options.SHOW_FPS";
        H = "sc";
        j = " on block ";
        B = "interpolation";
        Ga = ".item";
        N = "paleOakFence";
        s = "' for variant: '";
        z = 0.14000000059604645F /* ZKM Deobf */;
        ja = "deepslate_iron_ore";
        W = "GL11.GL_OUT_OF_MEMORY";
        f = "rain";
        a = "texture_height";
        l = "profile.";
        m = "logStripped";
        U = "mossBlock";
        r = "mirror_uv";
        G = "time";
        I = "Interrupted";
        k = "south";
        P = "Shadow map distance: ";
        Z = "Piece2.Shape15";
        L = "stop";
        S = 413 /* ZKM Deobf Int */;
        V = "textures/gui/presets/drought.png";
        e = "keyTextShadow";
        n = "dirtSize";
        M = "F1";
        X = "Java: ";
        C = "commands.replaceitem.success";
        Q = "Hey, that tickles! Hehehe!";
        v = "DECLINED";
        Aa = "commands.scoreboard.teams.empty.alreadyEmpty";
        E = "timezone_";
    }
}

