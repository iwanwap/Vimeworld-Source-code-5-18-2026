...
final class CollationParsedRuleBuilder {
    ...
    private void copyRangeFromUCA(BuildTable t2, int start, int end) {
        ...
        for (u2 = start; u2 <= end; ++u2) {
            int CE2 = t2.m_mapping_.getValue(u2);
            if (CE2 != -268435456 && (!CollationParsedRuleBuilder.isContractionTableElement(CE2) || CollationParsedRuleBuilder.getCE(t2.m_contractions_, CE2, 0) != -268435456)) continue;
        ...
        while (conts[offset] != '\u0000') {
            ...
            if (tailoredCE != -268435456) {
                ...
                if (needToAdd) {
                    ...
                    while ((CE2 = this.m_utilColEIter_.next()) != -1) {
                        this.m_utilElement_.m_CEs_[this.m_utilElement_.m_CELength_++] = CE2;
                    }
                    this.addAnElement(t2, this.m_utilElement_);
                }
    ...
    private final int getCaseBits(String src) throws Exception {
        ...
        for (int i2 = 0; i2 < src.length(); ++i2) {
            ...
            char ch2 = src.charAt(i2);
            if (UCharacter.isLowerCase(ch2)) {
                ++lCount;
                continue;
            }
            if (CollationParsedRuleBuilder.toSmallKana(ch2) != ch2 || CollationParsedRuleBuilder.toLargeKana(ch2) == ch2) continue;
            ++lCount;
        }
    ...
    private static final char toLargeKana(char ch2) {
        if ('\u3042' < ch2 && ch2 < '\u30ef') {
            switch (ch2 - 12288) {
                case 65: 
                case 67: 
    ...
    private static final char toSmallKana(char ch2) {
        if ('\u3042' < ch2 && ch2 < '\u30ef') {
            switch (ch2 - 12288) {
                case 66: 
                case 68: 
    ...
    private int getFirstCE(char ch2) {
        this.m_utilColEIter_.setText(UCharacter.toString(ch2));
    ...
    private static int setMaxJamoExpansion(char ch2, int endexpansion, byte expansionsize, MaxJamoExpansionTable maxexpansion) {
        boolean isV = true;
        if (ch2 >= '\u1100' && ch2 <= '\u1112') {
        ...
        if (ch2 >= '\u1161' && ch2 <= '\u1175' && maxexpansion.m_maxVSize_ < expansionsize) {
    ...
    private int addPrefix(BuildTable t2, int CE2, Elements element) {
        int j2;
        ...
        for (j2 = 1; j2 < size; ++j2) {
            char ch2 = element.m_prefixChars_.charAt(j2 + element.m_prefix_);
            if (UTF16.isTrailSurrogate(ch2)) continue;
            CollationParsedRuleBuilder.unsafeCPSet(t2.m_unsafeCP_, ch2);
        }
        ...
        if (!CollationParsedRuleBuilder.isPrefix(CE2)) {
            int firstContractionOffset = CollationParsedRuleBuilder.addContraction(contractions, 0xFFFFFF, '\u0000', CE2);
            ...
            CollationParsedRuleBuilder.addContraction(contractions, firstContractionOffset, '\uffff', CE2);
            CE2 = CollationParsedRuleBuilder.constructSpecialCE(11, firstContractionOffset);
        } else {
            char ch3 = element.m_prefixChars_.charAt(element.m_prefix_);
            int position = CollationParsedRuleBuilder.findCP(contractions, CE2, ch3);
            if (position > 0) {
                int eCE = CollationParsedRuleBuilder.getCE(contractions, CE2, position);
                int newCE = CollationParsedRuleBuilder.processContraction(contractions, element, eCE);
                CollationParsedRuleBuilder.setContraction(contractions, CE2, position, ch3, newCE);
            } else {
                CollationParsedRuleBuilder.processContraction(contractions, element, -268435456);
                CollationParsedRuleBuilder.insertContraction(contractions, CE2, ch3, element.m_mapCE_);
            }
        }
    ...
    private static final boolean isContraction(int CE2) {
        return CollationParsedRuleBuilder.isSpecial(CE2) && CollationParsedRuleBuilder.getCETag(CE2) == 2;
    }

    private static final boolean isPrefix(int CE2) {
        return CollationParsedRuleBuilder.isSpecial(CE2) && CollationParsedRuleBuilder.getCETag(CE2) == 11;
    }

    private static final boolean isSpecial(int CE2) {
        return (CE2 & 0xF0000000) == -268435456;
    }

    private static final int getCETag(int CE2) {
    ...
    private static final int constructSpecialCE(int tag, int CE2) {
        return 0xF0000000 | tag << 24 | CE2 & 0xFFFFFF;
    }
    ...
    private static final boolean isContractionTableElement(int CE2) {
        return CollationParsedRuleBuilder.isSpecial(CE2) && (CollationParsedRuleBuilder.getCETag(CE2) == 2 || CollationParsedRuleBuilder.getCETag(CE2) == 11);
    }
    ...
    private static final int finalizeAddition(BuildTable t2, Elements element) {
        ...
        if (element.m_mapCE_ == 0) {
            for (int i2 = 0; i2 < element.m_cPoints_.length(); ++i2) {
                char ch2 = element.m_cPoints_.charAt(i2);
                if (UTF16.isTrailSurrogate(ch2)) continue;
                CollationParsedRuleBuilder.unsafeCPSet(t2.m_unsafeCP_, ch2);
            }
        }
        if (element.m_cPoints_.length() - element.m_cPointsOffset_ > 1) {
            ...
            CE2 = t2.m_mapping_.getValue(cp2);
            CE2 = CollationParsedRuleBuilder.addContraction(t2, CE2, element);
        } else {
            CE2 = t2.m_mapping_.getValue(element.m_cPoints_.charAt(element.m_cPointsOffset_));
            if (CE2 != -268435456) {
                if (CollationParsedRuleBuilder.isContractionTableElement(CE2)) {
                    if (!CollationParsedRuleBuilder.isPrefix(element.m_mapCE_)) {
                        CollationParsedRuleBuilder.setContraction(t2.m_contractions_, CE2, 0, '\u0000', element.m_mapCE_);
                        CollationParsedRuleBuilder.changeLastCE(t2.m_contractions_, CE2, element.m_mapCE_);
                    }
                } else {
                    t2.m_mapping_.setValue(element.m_cPoints_.charAt(element.m_cPointsOffset_), element.m_mapCE_);
                    if (element.m_prefixChars_ != null && element.m_prefixChars_.length() > 0 && CollationParsedRuleBuilder.getCETag(CE2) != 10) {
                        Elements origElem = new Elements();
                        origElem.m_prefixChars_ = null;
                        origElem.m_cPoints_ = origElem.m_uchars_ = element.m_cPoints_;
                        origElem.m_CEs_[0] = CE2;
                        origElem.m_mapCE_ = CE2;
    ...
    private static int addContraction(BuildTable t2, int CE2, Elements element) {
        ContractionTable contractions = t2.m_contractions_;
        ...
        if (cpsize < element.m_cPoints_.length()) {
            ...
            if (!CollationParsedRuleBuilder.isContraction(CE2)) {
                int firstContractionOffset = CollationParsedRuleBuilder.addContraction(contractions, 0xFFFFFF, '\u0000', CE2);
                int newCE = CollationParsedRuleBuilder.processContraction(contractions, element, -268435456);
                CollationParsedRuleBuilder.addContraction(contractions, firstContractionOffset, element.m_cPoints_.charAt(element.m_cPointsOffset_), newCE);
                CollationParsedRuleBuilder.addContraction(contractions, firstContractionOffset, '\uffff', CE2);
                CE2 = CollationParsedRuleBuilder.constructSpecialCE(2, firstContractionOffset);
            } else {
                int position = CollationParsedRuleBuilder.findCP(contractions, CE2, element.m_cPoints_.charAt(element.m_cPointsOffset_));
                if (position > 0) {
                    int eCE = CollationParsedRuleBuilder.getCE(contractions, CE2, position);
                    int newCE = CollationParsedRuleBuilder.processContraction(contractions, element, eCE);
                    CollationParsedRuleBuilder.setContraction(contractions, CE2, position, element.m_cPoints_.charAt(element.m_cPointsOffset_), newCE);
                } else {
                    int newCE = CollationParsedRuleBuilder.processContraction(contractions, element, -268435456);
                    CollationParsedRuleBuilder.insertContraction(contractions, CE2, element.m_cPoints_.charAt(element.m_cPointsOffset_), newCE);
                }
            }
            element.m_cPointsOffset_ -= cpsize;
            t2.m_mapping_.setValue(cp2, CE2);
        } else if (!CollationParsedRuleBuilder.isContraction(CE2)) {
            t2.m_mapping_.setValue(cp2, element.m_mapCE_);
        } else {
            CollationParsedRuleBuilder.changeContraction(contractions, CE2, '\u0000', element.m_mapCE_);
            CollationParsedRuleBuilder.changeContraction(contractions, CE2, '\uffff', element.m_mapCE_);
        }
        return CE2;
    }
    ...
    private static int findCE(ContractionTable table, int element, char ch2) {
        if (table == null) {
            return -268435456;
        }
        BasicContractionTable tbl = CollationParsedRuleBuilder.getBasicContractionTable(table, element);
        ...
        int position = CollationParsedRuleBuilder.findCP(tbl, ch2);
    ...
    private int constructTable(ContractionTable table) {
        ...
        for (i2 = 0; i2 < tsize; ++i2) {
            int j2;
            BasicContractionTable bct = table.m_elements_.get(i2);
            ...
            for (j2 = 1; j2 < size; ++j2) {
                char ch2 = bct.m_codePoints_.charAt(j2);
                char cc2 = (char)(UCharacter.getCombiningClass(ch2) & 0xFF);
                if (cc2 > ccMax) {
                    ccMax = cc2;
                }
                if (cc2 < ccMin) {
                ...
                cpPointer.append(ch2);
                CEPointer.add(bct.m_CEs_.get(j2));
            }
        ...
        for (i2 = 0; i2 <= 0x10FFFF; ++i2) {
            int CE2 = table.m_mapping_.getValue(i2);
            if (!CollationParsedRuleBuilder.isContractionTableElement(CE2)) continue;
            CE2 = CollationParsedRuleBuilder.constructSpecialCE(CollationParsedRuleBuilder.getCETag(CE2), table.m_offsets_.get(CollationParsedRuleBuilder.getContractionOffset(CE2)));
            table.m_mapping_.setValue(i2, CE2);
        }
        return position;
    }
    ...
    private static final boolean isJamo(char ch2) {
        return ch2 >= '\u1100' && ch2 <= '\u1112' || ch2 >= '\u1175' && ch2 <= '\u1161' || ch2 >= '\u11a8' && ch2 <= '\u11c2';
    }
    ...
    private void processUCACompleteIgnorables(BuildTable t2) {
        ...
        while (trieiterator.next(element)) {
            ...
            for (int start = element.start; start < limit; ++start) {
                int CE2 = t2.m_mapping_.getValue(start);
                if (CE2 != -268435456) continue;
                this.m_utilElement_.m_prefix_ = 0;
    ...
    static class InverseUCA {
        ...
        final int getCEStrengthDifference(int CE2, int contCE, int prevCE, int prevContCE) {
            int strength;
            for (strength = 2; ((prevCE & STRENGTH_MASK_[strength]) != (CE2 & STRENGTH_MASK_[strength]) || (prevContCE & STRENGTH_MASK_[strength]) != (contCE & STRENGTH_MASK_[strength])) && strength != 0; --strength) {
            }
            return strength;
        }
