...
extends SearchIterator {
    ...
    protected int handleNext(int start) {
        if (this.m_pattern_.m_CELength_ == 0) {
            ...
            char ch2 = this.targetText.current();
            char ch22 = this.targetText.next();
            this.m_matchedIndex_ = ch22 == '\uffff' ? -1 : this.targetText.getIndex();
            if (UTF16.isLeadSurrogate(ch2) && UTF16.isTrailSurrogate(ch22)) {
    ...
    protected int handlePrevious(int start) {
        if (this.m_pattern_.m_CELength_ == 0) {
            this.matchLength = 0;
            this.targetText.setIndex(start);
            char ch2 = this.targetText.previous();
            if (ch2 == '\uffff') {
                this.m_matchedIndex_ = -1;
            } else {
                this.m_matchedIndex_ = this.targetText.getIndex();
                if (UTF16.isTrailSurrogate(ch2) && UTF16.isLeadSurrogate(this.targetText.previous())) {
                    this.m_matchedIndex_ = this.targetText.getIndex();
                }
            }
        } else {
    ...
    private final char getFCD(CharacterIterator str, int offset) {
        char ch2 = str.setIndex(offset);
        if (ch2 < '\u0180') {
            return (char)this.m_nfcImpl_.getFCD16FromBelow180(ch2);
        }
        if (this.m_nfcImpl_.singleLeadMightHaveNonZeroFCD16(ch2)) {
            if (!Character.isHighSurrogate(ch2)) {
                return (char)this.m_nfcImpl_.getFCD16FromNormData(ch2);
            }
            char c2 = str.next();
            if (Character.isLowSurrogate(c2)) {
                return (char)this.m_nfcImpl_.getFCD16FromNormData(Character.toCodePoint(ch2, c2));
            }
        }
        return '\u0000';
    }
