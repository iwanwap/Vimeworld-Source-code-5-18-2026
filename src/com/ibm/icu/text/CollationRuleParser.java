...
final class CollationRuleParser {
    ...
    private int parseNextTokenInternal(boolean startofrules) throws ParseException {
        ...
        while (this.m_current_ < limit) {
            char ch2 = this.m_source_.charAt(this.m_current_);
            if (inquote) {
                if (ch2 == '\'') {
                    inquote = false;
            ...
            } else if (!PatternProps.isWhiteSpace(ch2)) {
                switch (ch2) {
                    case '=': {
                        if (newstrength != -1) {
                    ...
                    case '\'': {
                        ...
                        wasinquote = true;
                        ++this.m_current_;
                        ch2 = this.m_source_.charAt(this.m_current_);
                        if (ch2 != '\'') break;
                        this.m_source_.append(ch2);
                    ...
                    case '|': {
                        ...
                        do {
                            ++this.m_current_;
                        } while (PatternProps.isWhiteSpace(ch2 = this.m_source_.charAt(this.m_current_)));
                        break;
                    }
                    case '-': {
                        if (newstrength != -1) {
                    ...
                    case '#': {
                        do {
                            ++this.m_current_;
                        } while (!CollationRuleParser.isCharNewLine(ch2 = this.m_source_.charAt(this.m_current_)));
                        break;
                    }
                    case '!': {
                        break;
                    ...
                    default: {
                        ...
                        if (CollationRuleParser.isSpecialChar(ch2) && !inquote) {
                            CollationRuleParser.throwParseException(this.m_rules_, this.m_current_);
                        }
                        if (ch2 == '\u0000' && this.m_current_ + 1 == limit) break;
                        if (inchars) {
                            if (this.m_parsedToken_.m_charsLen_ == 0) {
                                this.m_parsedToken_.m_charsOffset_ = this.m_current_;
                            }
    ...
    private static final boolean isSpecialChar(char ch2) {
        return ch2 <= '/' && ch2 >= ' ' || ch2 <= '?' && ch2 >= ':' || ch2 <= '`' && ch2 >= '[' || ch2 <= '~' && ch2 >= '}' || ch2 == '{';
    }
