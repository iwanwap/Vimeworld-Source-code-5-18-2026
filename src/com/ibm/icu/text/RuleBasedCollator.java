...
extends Collator {
    ...
    private void addSpecial(contContext c2, StringBuilder buffer, int CE2) {
        block16: {
            int newCE;
            int offset;
            StringBuilder b2;
            block15: {
                ...
                offset = (CE2 & 0xFFFFFF) - c2.coll.m_contractionOffset_;
                newCE = c2.coll.m_contractionCE_[offset];
                if (newCE != -268435456) {
                    if (RuleBasedCollator.isSpecial(CE2) && RuleBasedCollator.getTag(CE2) == 2 && RuleBasedCollator.isSpecial(newCE) && RuleBasedCollator.getTag(newCE) == 11 && c2.addPrefixes) {
                    ...
                    if (buffer.length() > 1) {
                        ...
                        if (c2.expansions != null && RuleBasedCollator.isSpecial(CE2) && RuleBasedCollator.getTag(CE2) == 1) {
                            c2.expansions.add(buffer.toString());
                        }
                    }
                }
                ++offset;
                if (RuleBasedCollator.getTag(CE2) != 11 || !c2.addPrefixes) break block15;
            ...
            if (RuleBasedCollator.getTag(CE2) != 2) break block16;
            while (c2.coll.m_contractionIndex_[offset] != '\uffff') {
                b2.delete(0, b2.length());
                b2.append((CharSequence)buffer);
                newCE = c2.coll.m_contractionCE_[offset];
    ...
    private void processSpecials(contContext c2) {
        ...
        while (trieiterator.next(element)) {
            ...
            int CE2 = element.value;
            StringBuilder contraction = new StringBuilder(internalBufferSize);
            if (!RuleBasedCollator.isSpecial(CE2)) continue;
            if (RuleBasedCollator.getTag(CE2) == 11 && c2.addPrefixes || RuleBasedCollator.getTag(CE2) == 2) {
                while (start < limit) {
                    ...
                    this.addSpecial(c2, contraction, CE2);
                    ++start;
                }
                continue;
            }
            if (c2.expansions == null || RuleBasedCollator.getTag(CE2) != 1) continue;
            while (start < limit) {
    ...
    final boolean isUnsafe(char ch2) {
        byte value;
        if (ch2 < this.m_minUnsafe_) {
            return false;
        }
        if (ch2 >= '\u2100') {
            if (UTF16.isLeadSurrogate(ch2) || UTF16.isTrailSurrogate(ch2)) {
                return true;
            }
            ch2 = (char)(ch2 & 0x1FFF);
            ch2 = (char)(ch2 + 256);
        }
        return ((value = this.m_unsafe_[ch2 >> 3]) >> (ch2 & 7) & 1) != 0;
    }

    final boolean isContractionEnd(char ch2) {
        byte value;
        if (UTF16.isTrailSurrogate(ch2)) {
            return true;
        }
        if (ch2 < this.m_minContractionEnd_) {
            return false;
        }
        if (ch2 >= '\u2100') {
            ch2 = (char)(ch2 & 0x1FFF);
        ...
        return ((value = this.m_contractionEnd_[ch2 >> 3]) >> (ch2 & 7) & 1) != 0;
    }
    ...
    private static final char fixupUTF16(char ch2) {
        ch2 = ch2 >= '\ue000' ? (char)(ch2 - 2048) : (char)(ch2 + 8192);
        return ch2;
    }
    ...
    private final void addLatinOneEntry(char ch2, int CE2, shiftValues sh2) {
        int primary1 = 0;
        int primary2 = 0;
        int secondary = 0;
        int tertiary = 0;
        boolean continuation = RuleBasedCollator.isContinuation(CE2);
        boolean reverseSecondary = false;
        if (!continuation) {
            tertiary = CE2 & this.m_mask3_;
            tertiary ^= this.m_caseSwitch_;
            reverseSecondary = true;
        } else {
            tertiary = (byte)(CE2 & 0xFFFFFF3F);
            tertiary &= 0x3F;
            reverseSecondary = false;
        }
        secondary = (CE2 >>>= 8) & 0xFF;
        primary2 = (CE2 >>>= 8) & 0xFF;
        primary1 = CE2 >>> 8;
        if (primary1 != 0) {
        ...
        if (primary2 != 0) {
            if (sh2.primShift < 0) {
                this.latinOneCEs_[ch2] = -16777216;
                this.latinOneCEs_[this.latinOneTableLen_ + ch2] = -16777216;
                this.latinOneCEs_[2 * this.latinOneTableLen_ + ch2] = -16777216;
                return;
            }
            char c3 = ch2;
...
lbl80:
    ...
    private int getLatinOneContraction(int strength, int CE2, String s2) {
        int len = s2.length();
        int UCharOffset = (CE2 & 0xFFF) - this.m_contractionOffset_;
        int offset = 1;
        int latinOneOffset = (CE2 & 0xFFF000) >>> 12;
        char schar = '\u0000';
        char tchar = '\u0000';
        while (true) {
