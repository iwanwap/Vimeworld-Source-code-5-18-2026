...
extends RealmsScreen {
    ...
    public void postInit() {
        ...
        this.configureButton = RealmsMainScreen.newButton((int)2, (int)(this.width() / 2 + 6), (int)(this.height() - 52), (int)154, (int)20, (String)RealmsMainScreen.getLocalizedString((String)"mco.selectServer.configure"));
        this.buttonsAdd(this.configureButton);
        this.leaveButton = RealmsMainScreen.newButton((int)3, (int)(this.width() / 2 - 154), (int)(this.height() - 28), (int)102, (int)20, (String)RealmsMainScreen.getLocalizedString((String)"mco.selectServer.leave"));
        this.buttonsAdd(this.leaveButton);
        this.buyButton = RealmsMainScreen.newButton((int)4, (int)(this.width() / 2 - 48), (int)(this.height() - 28), (int)102, (int)20, (String)RealmsMainScreen.getLocalizedString((String)"mco.selectServer.buy"));
            ...
            public void run() {
                try {
                    ...
                    if (client.createTrial().booleanValue()) {
                        ...
                        Realms.setScreen((RealmsScreen)new RealmsGenericErrorScreen(RealmsScreen.getLocalizedString((String)"mco.trial.unavailable"), mainScreen));
                    }
                }
                catch (RealmsServiceException e2) {
                    LOGGER.error("Trials wasn't available: " + e2.toString());
                    Realms.setScreen((RealmsScreen)new RealmsGenericErrorScreen(e2, (RealmsScreen)RealmsMainScreen.this));
                }
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (versionResponse.equals((Object)RealmsClient.CompatibleVersionResponse.OUTDATED)) {
                            Realms.setScreen((RealmsScreen)(realmsGenericErrorScreen = new RealmsClientOutdatedScreen(RealmsMainScreen.this.lastScreen, true)));
                            return;
                        }
                        if (versionResponse.equals((Object)RealmsClient.CompatibleVersionResponse.OTHER)) {
                            Realms.setScreen((RealmsScreen)(realmsGenericErrorScreen = new RealmsClientOutdatedScreen(RealmsMainScreen.this.lastScreen, false)));
                            return;
                        }
                    }
                    catch (RealmsServiceException e2) {
                        ...
                        Realms.setScreen((RealmsScreen)new RealmsGenericErrorScreen(e2, RealmsMainScreen.this.lastScreen));
                        return;
                    }
                    catch (IOException e3) {
                        checkedMcoAvailability = false;
                        LOGGER.error("Couldn't connect to realms: ", e3.getMessage());
                        Realms.setScreen((RealmsScreen)new RealmsGenericErrorScreen(e3.getMessage(), RealmsMainScreen.this.lastScreen));
                        return;
                    }
                    boolean retry = false;
    ...
    public void keyPressed(char ch2, int eventKey) {
        switch (eventKey) {
            ...
            default: {
                if (this.mchars[this.mindex] == ch2) {
                    ++this.mindex;
                ...
                if (this.schars[this.sindex] == ch2) {
                    ++this.sindex;
    ...
    private void drawRealmsLogo(int x2, int y2) {
        ...
        RealmsScreen.blit((int)(x2 * 2), (int)(y2 * 2 - 5), (float)0.0f, (float)0.0f, (int)200, (int)50, (float)200.0f, (float)50.0f);
        GL11.glPopMatrix();
    }
    ...
    private void drawInvitationPendingIcon(int xm2, int ym2) {
        ...
        GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
        GL11.glPushMatrix();
        RealmsScreen.blit((int)baseX, (int)2, (float)(hovering ? 16.0f : 0.0f), (float)0.0f, (int)15, (int)25, (float)31.0f, (float)25.0f);
        GL11.glPopMatrix();
        if (pendingInvitesCount != 0) {
            ...
            GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
            GL11.glPushMatrix();
            RealmsScreen.blit((int)(baseX + 4), (int)(12 + yOff), (float)spritePos, (float)(hovering ? 8.0f : 0.0f), (int)8, (int)8, (float)48.0f, (float)16.0f);
            GL11.glPopMatrix();
        }
        if (hovering) {
            int rx2 = xm2 + 12;
            int ry2 = ym2 - 12;
            String message = pendingInvitesCount == 0 ? RealmsMainScreen.getLocalizedString((String)"mco.invites.nopending") : RealmsMainScreen.getLocalizedString((String)"mco.invites.pending");
            int width = this.fontWidth(message);
            this.fillGradient(rx2 - 3, ry2 - 3, rx2 + width + 3, ry2 + 8 + 3, -1073741824, -1073741824);
            this.fontDrawShadow(message, rx2, ry2, -1);
        }
    }
    ...
    public void play(RealmsServer server) {
        if (server != null) {
            ...
            if (server.resourcePackUrl != null && server.resourcePackHash != null) {
                ...
                this.saveListScrollPosition();
                String line2 = RealmsMainScreen.getLocalizedString((String)"mco.configure.world.resourcepack.question.line1");
                String line3 = RealmsMainScreen.getLocalizedString((String)"mco.configure.world.resourcepack.question.line2");
    ...
    private void drawExpired(int x2, int y2, int xm2, int ym2) {
        ...
        GL11.glPushMatrix();
        GL11.glScalef(0.5f, 0.5f, 0.5f);
        RealmsScreen.blit((int)(x2 * 2), (int)(y2 * 2), (float)0.0f, (float)0.0f, (int)15, (int)15, (float)15.0f, (float)15.0f);
        GL11.glPopMatrix();
    ...
    protected void renderMousehoverTooltip(String msg, int x2, int y2) {
        ...
        for (String s2 : msg.split("\n")) {
            this.fillGradient(rx2 - 3, ry2 - (index == 0 ? 3 : 0) + index, rx2 + width + 3, ry2 + 8 + 3 + index, -1073741824, -1073741824);
            this.fontDrawShadow(s2, rx2, ry2 + index, 0xFFFFFF);
            index += 10;
        }
    }
    ...
    extends RealmsScrolledSelectionList {
        ...
        private void renderTrialItem(int i2, int x2, int y2) {
            int ry2 = y2 + 12;
            int index = 0;
            String msg = RealmsScreen.getLocalizedString((String)"mco.trial.message");
        ...
        private void renderMcoServerItem(int i2, int x2, int y2) {
            ...
            if (serverData.state == RealmsServer.State.UNINITIALIZED) {
                ...
                GL11.glEnable(3008);
                GL11.glPushMatrix();
                RealmsScreen.blit((int)(x2 + 10), (int)(y2 + 6), (float)0.0f, (float)0.0f, (int)40, (int)20, (float)40.0f, (float)20.0f);
                GL11.glPopMatrix();
                float scale = 0.5f + (1.0f + RealmsMth.sin((float)((float)RealmsMainScreen.this.animTick * 0.25f))) * 0.25f;
                int textColor = 0xFF000000 | (int)(127.0f * scale) << 16 | (int)(255.0f * scale) << 8 | (int)(127.0f * scale);
                RealmsMainScreen.this.drawCenteredString(RealmsScreen.getLocalizedString((String)"mco.selectServer.uninitialized"), x2 + 10 + 40 + 75, y2 + 12, textColor);
                return;
            }
