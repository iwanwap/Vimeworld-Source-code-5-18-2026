...
implements Reader {
    ...
    extends BinaryReader {
        ...
        private int readVarint32() throws IOException {
            ...
            if ((x2 ^= this.buffer[i2++] << 7) < 0) {
                x2 ^= 0xFFFFFF80;
            } else if ((x2 ^= this.buffer[i2++] << 14) >= 0) {
                x2 ^= 0x3F80;
            } else if ((x2 ^= this.buffer[i2++] << 21) < 0) {
                x2 ^= 0xFFE03F80;
            } else {
                byte y2 = this.buffer[i2++];
                x2 ^= y2 << 28;
                x2 ^= 0xFE03F80;
                if (y2 < 0 && this.buffer[i2++] < 0 && this.buffer[i2++] < 0 && this.buffer[i2++] < 0 && this.buffer[i2++] < 0 && this.buffer[i2++] < 0) {
        ...
        public long readVarint64() throws IOException {
            ...
            if ((y2 ^= buffer[i2++] << 7) < 0) {
                x2 = y2 ^ 0xFFFFFF80;
            } else if ((y2 ^= buffer[i2++] << 14) >= 0) {
                x2 = y2 ^ 0x3F80;
            } else if ((y2 ^= buffer[i2++] << 21) < 0) {
                x2 = y2 ^ 0xFFE03F80;
            } else if ((x2 = (long)y2 ^ (long)buffer[i2++] << 28) >= 0L) {
                x2 ^= 0xFE03F80L;
            } else if ((x2 ^= (long)buffer[i2++] << 35) < 0L) {
                x2 ^= 0xFFFFFFF80FE03F80L;
            } else if ((x2 ^= (long)buffer[i2++] << 42) >= 0L) {
                x2 ^= 0x3F80FE03F80L;
            } else if ((x2 ^= (long)buffer[i2++] << 49) < 0L) {
                x2 ^= 0xFFFE03F80FE03F80L;
            } else {
                x2 ^= (long)buffer[i2++] << 56;
                if ((x2 ^= 0xFE03F80FE03F80L) < 0L && (long)buffer[i2++] < 0L) {
                    throw InvalidProtocolBufferException.malformedVarint();
                }
            }
            this.pos = i2;
        ...
        private long readLittleEndian64_NoCheck() {
            ...
            byte[] buffer = this.buffer;
            this.pos = p2 + 8;
            return (long)buffer[p2] & 0xFFL | ((long)buffer[p2 + 1] & 0xFFL) << 8 | ((long)buffer[p2 + 2] & 0xFFL) << 16 | ((long)buffer[p2 + 3] & 0xFFL) << 24 | ((long)buffer[p2 + 4] & 0xFFL) << 32 | ((long)buffer[p2 + 5] & 0xFFL) << 40 | ((long)buffer[p2 + 6] & 0xFFL) << 48 | ((long)buffer[p2 + 7] & 0xFFL) << 56;
        }
