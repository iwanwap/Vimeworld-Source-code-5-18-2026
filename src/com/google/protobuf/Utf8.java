...
final class Utf8 {
    ...
    static int encodedLength(String string) {
        ...
        for (i2 = 0; i2 < utf16Length && string.charAt(i2) < '\u0080'; ++i2) {
        }
        while (i2 < utf16Length) {
            ...
            if (c2 < '\u0800') {
                ...
                utf8Length += Utf8.encodedLengthGeneral(string, i2);
                break;
            }
            ++i2;
        }
    ...
    private static int encodedLengthGeneral(String string, int start) {
        ...
        for (int i2 = start; i2 < utf16Length; ++i2) {
            ...
            utf8Length += 2;
            if ('\ud800' > c2 || c2 > '\udfff') continue;
            int cp2 = Character.codePointAt(string, i2);
            if (cp2 < 65536) {
    ...
    static int encode(String in2, byte[] out, int offset, int length) {
    ...
    static void encodeUtf8(String in2, ByteBuffer out) {
    ...
    private static int estimateConsecutiveAscii(ByteBuffer buffer, int index, int limit) {
        int i2;
        int lim = limit - 7;
        for (i2 = index; i2 < lim && (buffer.getLong(i2) & 0x8080808080808080L) == 0L; i2 += 8) {
        }
        return i2 - index;
    }
    ...
    static abstract class Processor {
        ...
        final String decodeUtf8Default(ByteBuffer buffer, int index, int size) throws InvalidProtocolBufferException {
            ...
            int resultPos = 0;
            for (offset = index; offset < limit && DecodeUtil.isOneByte(b2 = buffer.get(offset)); ++offset) {
            ...
            while (offset < limit) {
                ...
                if (DecodeUtil.isOneByte(byte1 = buffer.get(offset++))) {
                    ...
                    while (offset < limit && DecodeUtil.isOneByte(b3 = buffer.get(offset))) {
                        ++offset;
                        DecodeUtil.handleOneByte(b3, resultArr, resultPos++);
                    }
                    continue;
                }
        ...
        final void encodeUtf8(String in2, ByteBuffer out) {
            if (out.hasArray()) {
                int offset = out.arrayOffset();
                int endIndex = Utf8.encode(in2, out.array(), offset + out.position(), out.remaining());
                Java8Compatibility.position(out, endIndex - offset);
            } else if (out.isDirect()) {
                this.encodeUtf8Direct(in2, out);
            } else {
                this.encodeUtf8Default(in2, out);
            }
        }

        abstract void encodeUtf8Direct(String var1, ByteBuffer var2);

        final void encodeUtf8Default(String in2, ByteBuffer out) {
            int inIx;
            int inLength = in2.length();
            int outIx = out.position();
            try {
                char c2;
                for (inIx = 0; inIx < inLength && (c2 = in2.charAt(inIx)) < '\u0080'; ++inIx) {
                    out.put(outIx + inIx, (byte)c2);
                }
                if (inIx == inLength) {
                ...
                while (inIx < inLength) {
                    c2 = in2.charAt(inIx);
                    if (c2 < '\u0080') {
                        out.put(outIx, (byte)c2);
                    } else if (c2 < '\u0800') {
                        out.put(outIx++, (byte)(0xC0 | c2 >>> 6));
                        out.put(outIx, (byte)(0x80 | 0x3F & c2));
                    } else if (c2 < '\ud800' || '\udfff' < c2) {
                        out.put(outIx++, (byte)(0xE0 | c2 >>> 12));
                        out.put(outIx++, (byte)(0x80 | 0x3F & c2 >>> 6));
                        out.put(outIx, (byte)(0x80 | 0x3F & c2));
                    } else {
                        char low;
                        if (inIx + 1 == inLength || !Character.isSurrogatePair(c2, low = in2.charAt(++inIx))) {
    ...
    extends Processor {
        ...
        String decodeUtf8Direct(ByteBuffer buffer, int index, int size) throws InvalidProtocolBufferException {
            ...
            char[] resultArr = new char[size];
            int resultPos = 0;
            for (address = UnsafeUtil.addressOffset(buffer) + (long)index; address < addressLimit && DecodeUtil.isOneByte(b2 = UnsafeUtil.getByte(address)); ++address) {
                DecodeUtil.handleOneByte(b2, resultArr, resultPos++);
            }
            while (address < addressLimit) {
                ...
                if (DecodeUtil.isOneByte(byte1 = UnsafeUtil.getByte(address++))) {
                    byte b3;
                    DecodeUtil.handleOneByte(byte1, resultArr, resultPos++);
                    while (address < addressLimit && DecodeUtil.isOneByte(b3 = UnsafeUtil.getByte(address))) {
                        ++address;
                        DecodeUtil.handleOneByte(b3, resultArr, resultPos++);
                    }
                    continue;
        ...
        @Override
        int encodeUtf8(String in2, byte[] out, int offset, int length) {
            char c2;
            int inIx;
            long outIx = offset;
            long outLimit = outIx + (long)length;
            int inLimit = in2.length();
            if (inLimit > length || out.length - length < offset) {
                throw new ArrayIndexOutOfBoundsException("Failed writing " + in2.charAt(inLimit - 1) + " at index " + (offset + length));
            }
            for (inIx = 0; inIx < inLimit && (c2 = in2.charAt(inIx)) < '\u0080'; ++inIx) {
                UnsafeUtil.putByte(out, outIx++, (byte)c2);
            }
            if (inIx == inLimit) {
                return (int)outIx;
            }
            while (inIx < inLimit) {
                c2 = in2.charAt(inIx);
                if (c2 < '\u0080' && outIx < outLimit) {
                    UnsafeUtil.putByte(out, outIx++, (byte)c2);
                } else if (c2 < '\u0800' && outIx <= outLimit - 2L) {
                    UnsafeUtil.putByte(out, outIx++, (byte)(0x3C0 | c2 >>> 6));
                    UnsafeUtil.putByte(out, outIx++, (byte)(0x80 | 0x3F & c2));
                } else if ((c2 < '\ud800' || '\udfff' < c2) && outIx <= outLimit - 3L) {
                    UnsafeUtil.putByte(out, outIx++, (byte)(0x1E0 | c2 >>> 12));
                    UnsafeUtil.putByte(out, outIx++, (byte)(0x80 | 0x3F & c2 >>> 6));
                    UnsafeUtil.putByte(out, outIx++, (byte)(0x80 | 0x3F & c2));
                } else if (outIx <= outLimit - 4L) {
                    char low;
                    if (inIx + 1 == inLimit || !Character.isSurrogatePair(c2, low = in2.charAt(++inIx))) {
                    ...
                    if (!('\ud800' > c2 || c2 > '\udfff' || inIx + 1 != inLimit && Character.isSurrogatePair(c2, in2.charAt(inIx + 1)))) {
        ...
        @Override
        void encodeUtf8Direct(String in2, ByteBuffer out) {
            char c2;
            int inIx;
            ...
            if ((long)inLimit > outLimit - outIx) {
                throw new ArrayIndexOutOfBoundsException("Failed writing " + in2.charAt(inLimit - 1) + " at index " + out.limit());
            }
            for (inIx = 0; inIx < inLimit && (c2 = in2.charAt(inIx)) < '\u0080'; ++inIx) {
            ...
            while (inIx < inLimit) {
                ...
                if (c2 < '\u0080' && outIx < outLimit) {
                    ...
                    UnsafeUtil.putByte(outIx++, (byte)(0x3C0 | c2 >>> 6));
                    UnsafeUtil.putByte(outIx++, (byte)(0x80 | 0x3F & c2));
                } else if ((c2 < '\ud800' || '\udfff' < c2) && outIx <= outLimit - 3L) {
                    UnsafeUtil.putByte(outIx++, (byte)(0x1E0 | c2 >>> 12));
                    UnsafeUtil.putByte(outIx++, (byte)(0x80 | 0x3F & c2 >>> 6));
                    UnsafeUtil.putByte(outIx++, (byte)(0x80 | 0x3F & c2));
                } else if (outIx <= outLimit - 4L) {
                    char low;
                    if (inIx + 1 == inLimit || !Character.isSurrogatePair(c2, low = in2.charAt(++inIx))) {
        ...
        private static int unsafeEstimateConsecutiveAscii(byte[] bytes, long offset, int maxChars) {
            ...
            for (i2 = 0; i2 < unaligned; ++i2) {
                if (UnsafeUtil.getByte(bytes, offset++) >= 0) continue;
                return i2;
            }
            while (i2 + 8 <= maxChars && (UnsafeUtil.getLong((Object)bytes, UnsafeUtil.BYTE_ARRAY_BASE_OFFSET + offset) & 0x8080808080808080L) == 0L) {
                offset += 8L;
                i2 += 8;
            }
            while (i2 < maxChars) {
        ...
        private static int unsafeEstimateConsecutiveAscii(long address, int maxChars) {
            ...
            for (int j2 = unaligned = (int)(-address & 7L); j2 > 0; --j2) {
        ...
        private static int partialIsValidUtf8(byte[] bytes, long offset, int remaining) {
            ...
            while (true) {
                ...
                int byte1 = 0;
                while (remaining > 0) {
                    byte by2 = UnsafeUtil.getByte(bytes, offset++);
                    byte1 = by2;
                    if (by2 < 0) break;
                    --remaining;
                }
    ...
    extends Processor {
        ...
        String decodeUtf8(byte[] bytes, int index, int size) throws InvalidProtocolBufferException {
            ...
            char[] resultArr = new char[size];
            int resultPos = 0;
            for (offset = index; offset < limit && DecodeUtil.isOneByte(b2 = bytes[offset]); ++offset) {
                DecodeUtil.handleOneByte(b2, resultArr, resultPos++);
            }
            while (offset < limit) {
                byte byte1;
                if (DecodeUtil.isOneByte(byte1 = bytes[offset++])) {
                    byte b3;
                    DecodeUtil.handleOneByte(byte1, resultArr, resultPos++);
                    while (offset < limit && DecodeUtil.isOneByte(b3 = bytes[offset])) {
                        ++offset;
                        DecodeUtil.handleOneByte(b3, resultArr, resultPos++);
                    }
                    continue;
        ...
        int encodeUtf8(String in2, byte[] out, int offset, int length) {
            ...
            for (i2 = 0; i2 < utf16Length && i2 + j2 < limit && (c2 = in2.charAt(i2)) < '\u0080'; ++i2) {
                out[j2 + i2] = (byte)c2;
            }
            if (i2 == utf16Length) {
            ...
            while (i2 < utf16Length) {
                ...
                if (c2 < '\u0080' && j2 < limit) {
                    out[j2++] = (byte)c2;
                } else if (c2 < '\u0800' && j2 <= limit - 2) {
                    out[j2++] = (byte)(0x3C0 | c2 >>> 6);
                    out[j2++] = (byte)(0x80 | 0x3F & c2);
                } else if ((c2 < '\ud800' || '\udfff' < c2) && j2 <= limit - 3) {
                    out[j2++] = (byte)(0x1E0 | c2 >>> 12);
                    out[j2++] = (byte)(0x80 | 0x3F & c2 >>> 6);
                    out[j2++] = (byte)(0x80 | 0x3F & c2);
                } else if (j2 <= limit - 4) {
                    char low;
                    if (i2 + 1 == in2.length() || !Character.isSurrogatePair(c2, low = in2.charAt(++i2))) {
                        throw new UnpairedSurrogateException(i2 - 1, utf16Length);
                    }
                    int codePoint = Character.toCodePoint(c2, low);
                    out[j2++] = (byte)(0xF0 | codePoint >>> 18);
                    out[j2++] = (byte)(0x80 | 0x3F & codePoint >>> 12);
                    out[j2++] = (byte)(0x80 | 0x3F & codePoint >>> 6);
                    out[j2++] = (byte)(0x80 | 0x3F & codePoint);
                } else {
                    if (!('\ud800' > c2 || c2 > '\udfff' || i2 + 1 != in2.length() && Character.isSurrogatePair(c2, in2.charAt(i2 + 1)))) {
                        throw new UnpairedSurrogateException(i2, utf16Length);
                    }
