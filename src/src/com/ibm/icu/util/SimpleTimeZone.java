...
extends BasicTimeZone {
    ...
    public int getOffset(int era2, int year, int month, int day, int dayOfWeek, int millis) {
        if (month < 0 || month > 11) {
            throw new IllegalArgumentException();
        }
        return this.getOffset(era2, year, month, day, dayOfWeek, millis, Grego.monthLength(year, month));
    }

    public int getOffset(int era2, int year, int month, int day, int dayOfWeek, int millis, int monthLength) {
        if (month < 0 || month > 11) {
            throw new IllegalArgumentException();
        }
        return this.getOffset(era2, year, month, day, dayOfWeek, millis, Grego.monthLength(year, month), Grego.previousMonthLength(year, month));
    }

    private int getOffset(int era2, int year, int month, int day, int dayOfWeek, int millis, int monthLength, int prevMonthLength) {
        if (era2 != 1 && era2 != 0 || month < 0 || month > 11 || day < 1 || day > monthLength || dayOfWeek < 1 || dayOfWeek > 7 || millis < 0 || millis >= 86400000 || monthLength < 28 || monthLength > 31 || prevMonthLength < 28 || prevMonthLength > 31) {
            throw new IllegalArgumentException();
        }
        int result = this.raw;
        if (!this.useDaylight || year < this.startYear || era2 != 1) {
            return result;
        }
