...
extends NumberFormat {
        ...
        if (status[0]) {
            ...
            if (!this.parseBigDecimal && mult == 1 && this.digitList.isIntegral()) {
                if (this.digitList.decimalAt < 12) {
                    ...
                    n2 = big.bitLength() < 64 ? Long.valueOf(big.longValue()) : big;
                }
            } else {
                com.ibm.icu.math.BigDecimal big = this.digitList.getBigDecimalICU(status[1]);
                n2 = big;
                if (mult != 1) {
                    n2 = big.divide(com.ibm.icu.math.BigDecimal.valueOf(mult), this.mathContext);
                }
            }
        }
    ...
    static final int match(String text, int pos, String str) {
        ...
        while (i2 < str.length() && pos >= 0) {
            int ch2 = UTF16.charAt(str, i2);
            i2 += UTF16.getCharCount(ch2);
