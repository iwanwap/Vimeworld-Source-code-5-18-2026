...
extends DateFormat {
    ...
    protected String subFormat(char ch2, int count, int beginOffset, FieldPosition pos, DateFormatSymbols fmtData, Calendar cal) throws IllegalArgumentException {
        return this.subFormat(ch2, count, beginOffset, 0, DisplayContext.CAPITALIZATION_NONE, pos, cal);
    }

    protected String subFormat(char ch2, int count, int beginOffset, int fieldNum, DisplayContext capitalizationContext, FieldPosition pos, Calendar cal) {
        StringBuffer buf = new StringBuffer();
        this.subFormat(buf, ch2, count, beginOffset, fieldNum, capitalizationContext, pos, cal);
        return buf.toString();
    }

    protected void subFormat(StringBuffer buf, char ch2, int count, int beginOffset, int fieldNum, DisplayContext capitalizationContext, FieldPosition pos, Calendar cal) {
    ...
    protected void zeroPaddingNumber(NumberFormat nf2, StringBuffer buf, int value, int minDigits, int maxDigits) {
        if (this.useLocalZeroPaddingNumberFormat && value >= 0) {
            ...
            nf2.format(value, buf, new FieldPosition(-1));
        }
    }
    ...
    protected int subParse(String text, int start, char ch2, int count, boolean obeyCount, boolean allowNegative, boolean[] ambiguousYear, Calendar cal) {
        return this.subParse(text, start, ch2, count, obeyCount, allowNegative, ambiguousYear, cal, null);
    }

    protected int subParse(String text, int start, char ch2, int count, boolean obeyCount, boolean allowNegative, boolean[] ambiguousYear, Calendar cal, MessageFormat numericLeapMonthFormatter) {
        Number number = null;
