...
public final class CollationElementIterator {
    ...
    public int next() {
        ...
        do {
            ...
            if (this.m_collator_.m_isHiragana4_) {
                this.m_isCodePointHiragana_ = this.m_isCodePointHiragana_ && ch2 >= '\u3099' && ch2 <= '\u309c' || ch2 >= '\u3040' && ch2 <= '\u309e' && (ch2 <= '\u3094' || ch2 >= '\u309d');
            }
            if (!RuleBasedCollator.isSpecial(result = ch2 <= '\u00ff' ? this.m_collator_.m_trie_.getLatin1LinearValue(ch2) : this.m_collator_.m_trie_.getLeadValue(ch2))) {
                return result;
            }
            if (result != -268435456) {
                result = this.nextSpecial(this.m_collator_, result, ch2);
            }
            if (result != -268435456) continue;
            if (RuleBasedCollator.UCA_ != null && RuleBasedCollator.isSpecial(result = RuleBasedCollator.UCA_.m_trie_.getLeadValue(ch2))) {
                result = this.nextSpecial(RuleBasedCollator.UCA_, result, ch2);
            }
            if (result != -268435456) continue;
            result = this.nextImplicit(ch2);
        } while (result == 0 && ch2 >= '\uac00' && ch2 <= '\ud7af');
        return result;
    }
    ...
    public int previous() {
        ...
        do {
            ...
            ch2 = (char)ch_int;
            if (this.m_collator_.m_isHiragana4_) {
                boolean bl2 = this.m_isCodePointHiragana_ = ch2 >= '\u3040' && ch2 <= '\u309f';
            }
            if (this.m_collator_.isContractionEnd(ch2) && !this.isBackwardsStart()) {
                result = this.previousSpecial(this.m_collator_, -234881024, ch2);
                continue;
            }
            result = ch2 <= '\u00ff' ? this.m_collator_.m_trie_.getLatin1LinearValue(ch2) : this.m_collator_.m_trie_.getLeadValue(ch2);
            if (RuleBasedCollator.isSpecial(result)) {
                result = this.previousSpecial(this.m_collator_, result, ch2);
            }
            if (result != -268435456) continue;
            if (!this.isBackwardsStart() && this.m_collator_.isContractionEnd(ch2)) {
                result = -234881024;
            } else if (RuleBasedCollator.UCA_ != null) {
                result = RuleBasedCollator.UCA_.m_trie_.getLeadValue(ch2);
            }
            if (!RuleBasedCollator.isSpecial(result) || RuleBasedCollator.UCA_ == null) continue;
            result = this.previousSpecial(RuleBasedCollator.UCA_, result, ch2);
        } while (result == 0 && ch2 >= '\uac00' && ch2 <= '\ud7af');
        if (result == -268435456) {
            result = this.previousImplicit(ch2);
        }
        return result;
    }
    ...
    public void setOffset(int offset) {
        ...
        char ch2 = (char)ch_int;
        if (ch_int != -1 && this.m_collator_.isUnsafe(ch2)) {
            if (UTF16.isTrailSurrogate(ch2)) {
                ...
                while (this.m_source_.getIndex() > 0 && this.m_collator_.isUnsafe(ch2)) {
                    ch2 = (char)this.m_source_.previous();
                }
                this.updateInternalState();
                int prevoffset = 0;
    ...
    private int getCombiningClass(int ch2) {
        if (ch2 >= 768 && this.m_collator_.isUnsafe((char)ch2) || ch2 > 65535) {
            return m_nfcImpl_.getCC(m_nfcImpl_.getNorm16(ch2));
        }
        return 0;
    }
    ...
    private boolean FCDCheck(int ch2, int offset) {
        int fcd;
        boolean result = true;
        this.m_FCDStart_ = offset - 1;
        this.m_source_.setIndex(offset);
        if (ch2 < 384) {
            fcd = m_nfcImpl_.getFCD16FromBelow180(ch2);
        } else if (m_nfcImpl_.singleLeadMightHaveNonZeroFCD16(ch2)) {
            if (Character.isHighSurrogate((char)ch2)) {
                int c2 = this.m_source_.next();
                if (c2 < 0) {
                    fcd = 0;
                } else if (Character.isLowSurrogate((char)c2)) {
                    fcd = m_nfcImpl_.getFCD16FromNormData(Character.toCodePoint((char)ch2, (char)c2));
                } else {
                    this.m_source_.moveIndex(-1);
                    fcd = 0;
                }
            } else {
                fcd = m_nfcImpl_.getFCD16FromNormData(ch2);
            }
        } else {
            fcd = 0;
        }
        ...
        if (prevTrailCC == 0) {
            ...
            while (true) {
                if ((ch2 = this.m_source_.nextCodePoint()) < 0) {
                    offset = this.m_source_.getIndex();
                    break;
                }
                fcd = m_nfcImpl_.getFCD16(ch2);
                int leadCC = fcd >> 8;
                if (leadCC == 0) {
                    offset = this.m_source_.getIndex() - Character.charCount(ch2);
                    break;
                }
    ...
    private boolean FCDCheckBackwards(int ch2, int offset) {
        boolean result;
        block12: {
            int fcd;
            ...
            if (ch2 < 384) {
                fcd = m_nfcImpl_.getFCD16FromBelow180(ch2);
            } else if (!Character.isLowSurrogate((char)ch2)) {
                fcd = m_nfcImpl_.singleLeadMightHaveNonZeroFCD16(ch2) ? m_nfcImpl_.getFCD16FromNormData(ch2) : 0;
            } else {
                int c2 = this.m_source_.previous();
                if (c2 < 0) {
                    fcd = 0;
                } else if (Character.isHighSurrogate((char)c2)) {
                    ch2 = Character.toCodePoint((char)c2, (char)ch2);
                    fcd = m_nfcImpl_.getFCD16FromNormData(ch2);
                    --offset;
                } else {
                    this.m_source_.moveIndex(1);
            ...
            if (fcd != 0) {
                while (true) {
                    int leadCC;
                    if ((leadCC = fcd >> 8) == 0 || (ch2 = this.m_source_.previousCodePoint()) < 0) {
                        offset = this.m_source_.getIndex();
                        break block12;
                    }
                    fcd = m_nfcImpl_.getFCD16(ch2);
    ...
    private int previousChar() {
        ...
        int ch2 = this.m_source_.previous();
        if (ch2 < 192) {
            this.m_source_.next();
            return result;
        }
    ...
    private int nextDiscontiguous(RuleBasedCollator collator, int entryoffset) {
        block17: {
            ...
            int ch2 = this.currentChar();
            this.m_utilSkippedBuffer_.appendCodePoint(ch2);
            int prevCC = 0;
            int cc2 = this.getCombiningClass(ch2);
    ...
    private int nextContraction(RuleBasedCollator collator, int ce2) {
        ...
        while (true) {
            ...
            ++offset;
            while (ch2 > collator.m_contractionIndex_[offset]) {
                ++offset;
            }
            if (ch2 == collator.m_contractionIndex_[offset]) {
                ce2 = collator.m_contractionCE_[offset];
            } else {
                int sCC;
                int miss = ch2;
                if (UTF16.isLeadSurrogate((char)ch2) && !this.isEnd()) {
                    char surrNextChar = (char)this.nextChar();
                    if (UTF16.isTrailSurrogate(surrNextChar)) {
                        miss = UCharacterProperty.getRawSupplementary((char)ch2, surrNextChar);
                    } else {
                        this.previousChar();
                    }
                }
        ...
        if (this.m_collator_.m_isNumericCollation_) {
            ...
            block19: {
                ...
                do {
                    ...
                    char ch2 = (char)char32;
                    if (!UTF16.isLeadSurrogate(ch2) || this.isEnd()) continue;
                    char trail = (char)this.nextChar();
                    if (UTF16.isTrailSurrogate(trail)) {
                        char32 = UCharacterProperty.getRawSupplementary(ch2, trail);
                        continue;
                    }
                    this.goBackOne();
    ...
    private int nextSurrogate(char ch2) {
        int ch_int = this.nextChar();
        char nextch = (char)ch_int;
        if (ch_int != 65535 && UTF16.isTrailSurrogate(nextch)) {
            int codepoint = UCharacterProperty.getRawSupplementary(ch2, nextch);
    ...
    private int nextHangul(RuleBasedCollator collator, char ch2) {
        char L2 = (char)(ch2 - 44032);
        char T2 = (char)(L2 % 28);
        L2 = (char)(L2 / 28);
        char V2 = (char)(L2 % 21);
        L2 = (char)(L2 / 21);
        ...
        if (!this.m_collator_.m_isJamoSpecial_) {
            this.m_CEBuffer_[this.m_CEBufferSize_++] = collator.m_trie_.getLeadValue(L2);
            this.m_CEBuffer_[this.m_CEBufferSize_++] = collator.m_trie_.getLeadValue(V2);
            if (T2 != '\u11a7') {
                this.m_CEBuffer_[this.m_CEBufferSize_++] = collator.m_trie_.getLeadValue(T2);
            }
            this.m_CEBufferOffset_ = 1;
            return this.m_CEBuffer_[0];
        }
        this.m_buffer_.append(L2);
        this.m_buffer_.append(V2);
        if (T2 != '\u11a7') {
            this.m_buffer_.append(T2);
        }
        this.m_bufferOffset_ = 0;
        this.m_FCDLimit_ = this.m_source_.getIndex();
     ...
     */
    private int nextSpecial(RuleBasedCollator collator, int ce2, char ch2) {
        int codepoint = ch2;
            ...
            do {
                switch (RuleBasedCollator.getTag(ce2)) {
                    ...
                    case 5: {
                        ...
                        codepoint = UCharacterProperty.getRawSupplementary((char)ch2, trail);
                        break;
                    }
                    case 11: {
                    ...
                    case 7: {
                        int n8 = this.nextSurrogate((char)ch2);
                        return n8;
                    }
                    case 6: {
                        int n9 = this.nextHangul(collator, (char)ch2);
                        return n9;
                    }
                    case 4: {
                        int n10 = -268435456;
    ...
    private int previousContraction(RuleBasedCollator collator, int ce2, char ch2) {
        this.m_utilStringBuffer_.setLength(0);
        char prevch = (char)this.previousChar();
        boolean atStart = false;
        while (collator.isUnsafe(ch2)) {
            this.m_utilStringBuffer_.insert(0, ch2);
            ch2 = prevch;
            if (this.isBackwardsStart()) {
                atStart = true;
                break;
    ...
    private int previousDigit(RuleBasedCollator collator, int ce2, char ch2) {
        if (this.m_collator_.m_isNumericCollation_) {
            int digIndx;
            boolean nonZeroValReached;
            int collateVal;
            ...
            block20: {
                ...
                this.m_utilStringBuffer_.setLength(3);
                int char32 = ch2;
                if (UTF16.isTrailSurrogate((char)ch2) && !this.isBackwardsStart()) {
                    char lead = (char)this.previousChar();
                    if (UTF16.isLeadSurrogate(lead)) {
                        char32 = UCharacterProperty.getRawSupplementary(lead, (char)ch2);
                    } else {
                        this.goForwardOne();
                    }
                }
                ...
                do {
                    ...
                    if (!UTF16.isTrailSurrogate((char)ch2) || this.isBackwardsStart()) continue;
                    char lead = (char)this.previousChar();
                    if (UTF16.isLeadSurrogate(lead)) {
                        char32 = UCharacterProperty.getRawSupplementary(lead, (char)ch2);
                        continue;
    ...
    private int previousHangul(RuleBasedCollator collator, char ch2) {
        char L2 = (char)(ch2 - 44032);
        char T2 = (char)(L2 % 28);
        L2 = (char)(L2 / 28);
        char V2 = (char)(L2 % 21);
    ...
    private int previousSurrogate(char ch2) {
        if (this.isBackwardsStart()) {
            return -268435456;
        }
        char prevch = (char)this.previousChar();
        if (UTF16.isLeadSurrogate(prevch)) {
            return this.previousImplicit(UCharacterProperty.getRawSupplementary(prevch, ch2));
        }
        if (prevch != '\uffff') {
            this.nextChar();
        }
    ...
    private int previousSpecial(RuleBasedCollator collator, int ce2, char ch2) {
        do {
            switch (RuleBasedCollator.getTag(ce2)) {
                case 0: {
                    return ce2;
                ...
                case 2: {
                    ...
                    return this.previousContraction(collator, ce2, ch2);
                }
                case 12: {
                ...
                case 13: {
                    ce2 = this.previousDigit(collator, ce2, ch2);
                    break;
                }
                case 6: {
                    return this.previousHangul(collator, ch2);
                }
                case 7: {
                    return -268435456;
                }
                case 8: {
                    return this.previousSurrogate(ch2);
                }
                case 9: {
                    return this.previousImplicit(ch2);
                }
                case 10: {
                    return this.previousImplicit(ch2);
                }
