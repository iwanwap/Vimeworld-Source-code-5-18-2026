...
implements GameProfileRepository {
    ...
    private static final URL PROFILE_URL = HttpAuthenticationService.constantURL("https://sessionservernew.vimeworld.com/profiles");
    ...
    public void findProfilesByNames(String[] names, Agent agent, ProfileLookupCallback callback) {
        ...
        int page = 0;
        for (List request : Iterables.partition(criteria, 30)) {
            boolean failed;
            int failCount = 0;
            do {
                failed = false;
                try {
                    ProfileSearchResultsResponse response = this.authenticationService.makeRequest(PROFILE_URL, request, ProfileSearchResultsResponse.class);
                    failCount = 0;
