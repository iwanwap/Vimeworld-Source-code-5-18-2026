...
public class RealmsTasks {
                ...
                if (addressRetrieved) {
                    if (this.data.resourcePackUrl != null && this.data.resourcePackHash != null) {
                        try {
                            ...
                            Futures.addCallback(Realms.downloadResourcePack((String)this.data.resourcePackUrl, (String)this.data.resourcePackHash), new FutureCallback<Object>(){

                                @Override
                                public void onSuccess(@Nullable Object result) {
                                    net.minecraft.realms.RealmsServerAddress address = net.minecraft.realms.RealmsServerAddress.parseString((String)finalA.address);
                        ...
                        net.minecraft.realms.RealmsServerAddress address = net.minecraft.realms.RealmsServerAddress.parseString((String)a2.address);
    ...
    extends LongRunningTask {
        ...
        private final RealmsScreen lastScreen;
        private String title = RealmsScreen.getLocalizedString((String)"mco.reset.world.resetting.screen.title");
    ...
    extends LongRunningTask {
        ...
        @Override
        public void run() {
            this.setTitle(RealmsScreen.getLocalizedString((String)"mco.configure.world.closing"));
            RealmsClient client = RealmsClient.createRealmsClient();
            for (int i2 = 0; i2 < 25; ++i2) {
                if (this.aborted()) {
                    return;
    ...
    extends LongRunningTask {
        ...
        @Override
        public void run() {
            this.setTitle(RealmsScreen.getLocalizedString((String)"mco.configure.world.opening"));
            RealmsClient client = RealmsClient.createRealmsClient();
            for (int i2 = 0; i2 < 25; ++i2) {
                if (this.aborted()) {
                    return;
