    ...
    extends CodedInputStream {
                    ...
                    block8: {
                        block7: {
                            block5: {
                                ...
                                buffer = this.buffer;
                                if ((x2 = buffer[tempPos++]) >= 0) {
                                    this.pos = tempPos;
                                    return x2;
                                }
                                if (this.bufferSize - tempPos < 9) break block4;
                                if ((x2 ^= buffer[tempPos++] << 7) >= 0) break block5;
                                x2 ^= 0xFFFFFF80;
                                break block6;
                            }
                            if ((x2 ^= buffer[tempPos++] << 14) < 0) break block7;
                            x2 ^= 0x3F80;
                            break block6;
                        }
                        if ((x2 ^= buffer[tempPos++] << 21) >= 0) break block8;
                        x2 ^= 0xFFE03F80;
                        break block6;
                    }
                    byte y2 = buffer[tempPos++];
                    x2 ^= y2 << 28;
                    x2 ^= 0xFE03F80;
                    if (y2 < 0 && buffer[tempPos++] < 0 && buffer[tempPos++] < 0 && buffer[tempPos++] < 0 && buffer[tempPos++] < 0 && buffer[tempPos++] < 0) break block4;
                }
                this.pos = tempPos;
                return x2;
            }
                                    ...
                                    block8: {
                                        block7: {
                                            block5: {
                                                ...
                                                if (this.bufferSize == tempPos) break block4;
                                                buffer = this.buffer;
                                                if ((y2 = buffer[tempPos++]) >= 0) {
                                                    this.pos = tempPos;
                                                    return y2;
                                                }
                                                if (this.bufferSize - tempPos < 9) break block4;
                                                if ((y2 ^= buffer[tempPos++] << 7) >= 0) break block5;
                                                x2 = y2 ^ 0xFFFFFF80;
                                                break block6;
                                            }
                                            if ((y2 ^= buffer[tempPos++] << 14) < 0) break block7;
                                            x2 = y2 ^ 0x3F80;
                                            break block6;
                                        }
                                        if ((y2 ^= buffer[tempPos++] << 21) >= 0) break block8;
                                        x2 = y2 ^ 0xFFE03F80;
                                        break block6;
                                    }
                                    if ((x2 = (long)y2 ^ (long)buffer[tempPos++] << 28) < 0L) break block9;
                                    x2 ^= 0xFE03F80L;
                                    break block6;
                                }
                                if ((x2 ^= (long)buffer[tempPos++] << 35) >= 0L) break block10;
                                x2 ^= 0xFFFFFFF80FE03F80L;
                                break block6;
                            }
                            if ((x2 ^= (long)buffer[tempPos++] << 42) < 0L) break block11;
                            x2 ^= 0x3F80FE03F80L;
                            break block6;
                        }
                        if ((x2 ^= (long)buffer[tempPos++] << 49) >= 0L) break block12;
                        x2 ^= 0xFFFE03F80FE03F80L;
                        break block6;
                    }
                    x2 ^= (long)buffer[tempPos++] << 56;
                    if ((x2 ^= 0xFE03F80FE03F80L) < 0L && (long)buffer[tempPos++] < 0L) break block4;
                }
                this.pos = tempPos;
                return x2;
            }
        ...
        private byte[] readRawBytesSlowPathOneChunk(int size) throws IOException {
            ...
            if (sizeLeft < 4096 || sizeLeft <= StreamDecoder.available(this.input)) {
                ...
                this.pos = 0;
                this.bufferSize = 0;
                for (int tempPos = bufferedBytes; tempPos < bytes.length; tempPos += n2) {
                    n2 = StreamDecoder.read(this.input, bytes, tempPos, size - tempPos);
                    if (n2 == -1) {
                        throw InvalidProtocolBufferException.truncatedMessage();
                    }
        ...
        private List<byte[]> readRawBytesSlowPathRemainingChunks(int sizeLeft) throws IOException {
            ...
            while (sizeLeft > 0) {
                ...
                for (int tempPos = 0; tempPos < chunk.length; tempPos += n2) {
                    n2 = this.input.read(chunk, tempPos, chunk.length - tempPos);
                    if (n2 == -1) {
                        throw InvalidProtocolBufferException.truncatedMessage();
                    }
                    this.totalBytesRetired += n2;
    ...
    extends CodedInputStream {
                    ...
                    block8: {
                        block7: {
                            block5: {
                                ...
                                if ((x2 = UnsafeUtil.getByte(tempPos++)) >= 0) {
                                    this.pos = tempPos;
                                    return x2;
                                }
                                if (this.limit - tempPos < 9L) break block4;
                                if ((x2 ^= UnsafeUtil.getByte(tempPos++) << 7) >= 0) break block5;
                                x2 ^= 0xFFFFFF80;
                                break block6;
                            }
                            if ((x2 ^= UnsafeUtil.getByte(tempPos++) << 14) < 0) break block7;
                            x2 ^= 0x3F80;
                            break block6;
                        }
                        if ((x2 ^= UnsafeUtil.getByte(tempPos++) << 21) >= 0) break block8;
                        x2 ^= 0xFFE03F80;
                        break block6;
                    }
                    byte y2 = UnsafeUtil.getByte(tempPos++);
                    x2 ^= y2 << 28;
                    x2 ^= 0xFE03F80;
                    if (y2 < 0 && UnsafeUtil.getByte(tempPos++) < 0 && UnsafeUtil.getByte(tempPos++) < 0 && UnsafeUtil.getByte(tempPos++) < 0 && UnsafeUtil.getByte(tempPos++) < 0 && UnsafeUtil.getByte(tempPos++) < 0) break block4;
                }
                this.pos = tempPos;
                return x2;
            }
                                    ...
                                    block8: {
                                        block7: {
                                            block5: {
                                                tempPos = this.pos;
                                                if (this.limit == tempPos) break block4;
                                                if ((y2 = UnsafeUtil.getByte(tempPos++)) >= 0) {
                                                    this.pos = tempPos;
                                                    return y2;
                                                }
                                                if (this.limit - tempPos < 9L) break block4;
                                                if ((y2 ^= UnsafeUtil.getByte(tempPos++) << 7) >= 0) break block5;
                                                x2 = y2 ^ 0xFFFFFF80;
                                                break block6;
                                            }
                                            if ((y2 ^= UnsafeUtil.getByte(tempPos++) << 14) < 0) break block7;
                                            x2 = y2 ^ 0x3F80;
                                            break block6;
                                        }
                                        if ((y2 ^= UnsafeUtil.getByte(tempPos++) << 21) >= 0) break block8;
                                        x2 = y2 ^ 0xFFE03F80;
                                        break block6;
                                    }
                                    if ((x2 = (long)y2 ^ (long)UnsafeUtil.getByte(tempPos++) << 28) < 0L) break block9;
                                    x2 ^= 0xFE03F80L;
                                    break block6;
                                }
                                if ((x2 ^= (long)UnsafeUtil.getByte(tempPos++) << 35) >= 0L) break block10;
                                x2 ^= 0xFFFFFFF80FE03F80L;
                                break block6;
                            }
                            if ((x2 ^= (long)UnsafeUtil.getByte(tempPos++) << 42) < 0L) break block11;
                            x2 ^= 0x3F80FE03F80L;
                            break block6;
                        }
                        if ((x2 ^= (long)UnsafeUtil.getByte(tempPos++) << 49) >= 0L) break block12;
                        x2 ^= 0xFFFE03F80FE03F80L;
                        break block6;
                    }
                    x2 ^= (long)UnsafeUtil.getByte(tempPos++) << 56;
                    if ((x2 ^= 0xFE03F80FE03F80L) < 0L && (long)UnsafeUtil.getByte(tempPos++) < 0L) break block4;
                }
                this.pos = tempPos;
                return x2;
            }
        ...
        private ByteBuffer slice(long begin, long end) throws IOException {
            ...
            try {
                ...
                InvalidProtocolBufferException ex2 = InvalidProtocolBufferException.truncatedMessage();
                ex2.initCause(e2);
                throw ex2;
            }
            finally {
            ...
            if (size > 0 && size <= this.remaining()) {
                if (this.immutable && this.enableAliasing) {
                    ...
                    while (l2 > 0) {
                        ...
                        int bytesToCopy = Math.min(l2, (int)this.currentRemaining());
                        int idx = (int)(this.currentByteBufferPos - this.currentAddress);
        ...
        private void readRawBytesTo(byte[] bytes, int offset, int length) throws IOException {
            if (length >= 0 && length <= this.remaining()) {
                ...
                while (l2 > 0) {
                    ...
                    int bytesToCopy = Math.min(l2, (int)this.currentRemaining());
                    UnsafeUtil.copyMemory(this.currentByteBufferPos, bytes, (long)(length - l2 + offset), (long)bytesToCopy);
                    l2 -= bytesToCopy;
                    this.currentByteBufferPos += (long)bytesToCopy;
                }
                return;
        ...
        public void skipRawBytes(int length) throws IOException {
            if (length >= 0 && (long)length <= (long)(this.totalBufferSize - this.totalBytesRead) - this.currentByteBufferPos + this.currentByteBufferStartPos) {
                ...
                while (l2 > 0) {
                    ...
                    int rl2 = Math.min(l2, (int)this.currentRemaining());
                    l2 -= rl2;
                    this.currentByteBufferPos += (long)rl2;
                }
                return;
            }
            if (length < 0) {
