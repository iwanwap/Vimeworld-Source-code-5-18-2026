...
final class Native {
    ...
    public static int sendTo(int fd2, ByteBuffer buf, int pos, int limit, InetAddress addr, int port) throws IOException {
        int scopeId;
        byte[] address;
        ...
        return Native.sendTo(fd2, buf, pos, limit, address, scopeId, port);
    }
    ...
    public static int sendToAddress(int fd2, long memoryAddress, int pos, int limit, InetAddress addr, int port) throws IOException {
        int scopeId;
        byte[] address;
        if (addr instanceof Inet6Address) {
            address = addr.getAddress();
        ...
        return Native.sendToAddress(fd2, memoryAddress, pos, limit, address, scopeId, port);
    }
    ...
    public static void bind(int fd2, InetAddress addr, int port) throws IOException {
        NativeInetAddress address = Native.toNativeInetAddress(addr);
        Native.bind(fd2, address.address, address.scopeId, port);
    }
    ...
    public static boolean connect(int fd2, InetAddress addr, int port) throws IOException {
        NativeInetAddress address = Native.toNativeInetAddress(addr);
        return Native.connect(fd2, address.address, address.scopeId, port);
    }
